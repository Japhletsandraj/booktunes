...
This is the `backend/` directory of the repo. Every command in this document —
`alembic`, `pytest`, `docker compose`, `make` — is run from here, not from the
repo root.
...
cd booktunes/backend        # every command below runs from here
